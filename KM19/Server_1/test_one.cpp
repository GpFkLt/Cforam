...
int main(int argc, char** argv) {
    if (argc != 5) {
        std::cout << "Usage: ./ours_server1 <log_database_size> <access_times> <element_size> <tag_size>" << std::endl;
        return -1;
    }
    try {
        ...
        Server_1 server(io_context, 4007, N, element_size, tag_size); // Port 4000 for server 0, Port 6000 for server 1        
        server.Setup();  
