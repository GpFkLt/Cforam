...
// Function to get a zero-filled element
element_type get_zero_element(size_t element_size) {
...
void set_socket_timeout(boost::asio::ip::tcp::socket& socket, size_t timeout_sec) {
...
size_t findSharedLevel(size_t loc0, size_t loc1, size_t treeHeight) {
    size_t shiftTimes = 1;
    while (loc0 != loc1) {
        loc0 >>= 1;
        loc1 >>= 1;
...
// Convert sequential index to reverse lexicographical order for each level,
// returning the index for each level and its position within its parent node.
std::vector<std::pair<size_t, size_t>> toReverseLexicographicalOrder(size_t index, size_t L, size_t d) {
    // Convert the sequential index to base-d and reverse the digits
    std::vector<size_t> reverseDigits(L);
    for (size_t i = 0; i < L; ++i) {
        reverseDigits[i] = index % d;
        index /= d;
    }

    // Calculate the reverse lexicographical index for the last level (leaf level)
    size_t leafIndex = 0;
    for (size_t i = 0; i < L; ++i) {
        leafIndex = leafIndex * d + reverseDigits[i];
    }

    // Calculate the index for each level and its position within its parent node
    std::vector<std::pair<size_t, size_t>> result(L + 1);
    size_t currentIndex = leafIndex;

    for (size_t level = L; level > 0; --level) {
        size_t parentIndex = currentIndex / d;
        size_t childPos = currentIndex % d;
        result[level] = {currentIndex, childPos};
        currentIndex = parentIndex;
    }

    result[0] = {currentIndex, 0};  // Root node, no parent node

    return result;
}

// Given the leaf index at the last level, find the node ID at a specified level
size_t getNodeIdAtLevel(size_t leafIndex, size_t targetLevel, size_t L, size_t d) {
    size_t currentIndex = leafIndex;
    for (size_t level = L; level > targetLevel; --level) {
        currentIndex /= d;
    }
    return currentIndex;
}

// Function to get the path from the root to a given leaf node
std::vector<size_t> getPathFromRootToLeaf(size_t leafIndex, size_t L, size_t d) {
    ...
    for (size_t level = L; level >= 0; --level) {
        ...
        if (level == 0) break;  // No parent for the root node
        currentIndex /= d;
    }

    // Reverse the path to start from root to leaf
