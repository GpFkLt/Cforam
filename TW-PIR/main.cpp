...
int main(int argc, char** argv) {
    ...
    if (program_name == 0) {
        ...
        // Test the function
        size_t NNN = 1 << 20;
        std::vector<uint8_t> data(NNN);
        for (size_t i = 0; i < NNN; i++) {
            data[i] = static_cast<uint8_t>(i);
        }
        std::vector<uint8_t> data2(NNN);
        for (size_t i = 0; i < NNN; i++) {
            data2[i] = static_cast<uint8_t>(i);
        }
        std::chrono::duration<double> buildT;
        std::chrono::duration<double> buildT2;
        buildT = std::chrono::duration<double>::zero();
        buildT2 = std::chrono::duration<double>::zero();

        auto time0 = std::chrono::high_resolution_clock::now();
        for (size_t i = 8; i <= 20; i++) {
            data2 = xor_half_vector_avx(data2, element_size);
        }
        auto time1 = std::chrono::high_resolution_clock::now();
        for (size_t i = 8; i <= 20; i++) {
            data = xor_half_vector(data, element_size);;
        }
        auto time2 = std::chrono::high_resolution_clock::now();
        buildT += time1 - time0;
        buildT2 += time2 - time1;
        ...
        std::cout << buildT.count() << " sec" << std::endl;
        std::cout << buildT2.count() << " sec" << std::endl;
    }
