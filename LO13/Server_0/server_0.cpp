    ...
    socket_(io_context)
{
    ...
    Standard_Table.resize(count_even(ell_cuckoo - 1));
    for (size_t i = 0; i < count_even(ell_cuckoo - 1); i++) {
    ...
    if (max_level > ell_cuckoo - 1) {
        size_t tmp_cuckoo_num = count_even(max_level) - count_even(ell_cuckoo - 1);
        Cuckoo_Table.resize(tmp_cuckoo_num);
        size_t tmp_base_tabLen = baseCuckoo_tableLen << (ell_cuckoo & 1);
...
bool Server_0::try_insert_standard(std::vector<element_array_type>& element_array, std::unordered_map<addr_type, size_t>& pos_dict, element_type& ele, size_t write_pos) {
...
bool Server_0::try_insert_cuckoo(std::vector<element_array_type>& element_array, std::vector<std::unordered_map<addr_type, size_t>>& pos_dict_array, 
...
void Server_0::write_element_to_socket(element_array_type& data_array, size_t count) {
...
void Server_0::send_standard_table(size_t level, size_t rec_ele_num) {
...
void Server_0::send_cuckoo_table(size_t level, size_t rec_ele_num) {
...
void Server_0::receive_standard_table(size_t level, std::vector<element_array_type>& element_array) {
    size_t level_tabLen = parametr_c * (1 << (level - 1));
    assert(element_array.size() == level_tabLen);
    element_type ele(num_cell_per_element);
...
void Server_0::receive_cuckoo_table(size_t level, std::vector<element_array_type>& element_array) {
    size_t level_tabLen = static_cast<size_t>(parametr_c * (1 << (level - 1)) * (1 + cuckoo_epsilon));
    element_type ele(num_cell_per_element);
    for (size_t i = 0; i < lg_N; i++) {
        boost::asio::read(socket_, boost::asio::buffer(ele.data(), element_size));
...
void Server_0::receive_shuffle_resend_standard_table(size_t level) {
    ...
    clr(Stash);
    size_t level_tabLen;
    for (size_t i = 2; i < tmp_level; i += 2) {
        level_tabLen = parametr_c * (1 << (i - 1));
        assert(level_tabLen == Standard_Table[(i - 2) >> 1].size());
        for (size_t j = 0; j < Standard_Table[(i - 2) >> 1].size(); j++) {
            for (size_t k = 0; k < standard_BC; k++) {
                ele = read_data_to_cell(Standard_Table[(i - 2) >> 1][j], k, element_size);
                write_data(tmp_element_array, glo_index, element_size, ele);
                glo_index++;
            }
            clr(Standard_Table[(i - 2) >> 1][j]);
        }
    }
    /**
...
void Server_0::receive_shuffle_resend_cuckoo_table(size_t level) {
    ...
    clr(Stash);
    size_t level_tabLen;
    for (size_t i = 2; i < ell_cuckoo; i += 2) {
        level_tabLen = parametr_c * (1 << (i - 1));
        assert(level_tabLen == Standard_Table[(i - 2) >> 1].size());
        for (size_t j = 0; j < Standard_Table[(i - 2) >> 1].size(); j++) {
            for (size_t k = 0; k < standard_BC; k++) {
    ...
    for (size_t i = ell_cuckoo + (ell_cuckoo & 1); i < tmp_level; i += 2) {
        level_tabLen = static_cast<size_t>(parametr_c * (1 << (i - 1)) * (1 + cuckoo_epsilon));
        for (size_t k = 0; k < level_tabLen; k++) {
            ele = read_data_to_cell(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][0], k, element_size);
            write_data(tmp_element_array, glo_index, element_size, ele);
            glo_index++;
        }
        for (size_t k = 0; k < level_tabLen; k++) {
            ele = read_data_to_cell(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][1], k, element_size);
            write_data(tmp_element_array, glo_index, element_size, ele);
            glo_index++;
        }
        clr(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][0]);
        clr(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][1]);
    }
...
void Server_0::Setup() {
    size_t case_identifier = ((max_level & 1) << 1) | (ell_cuckoo >= max_level + 1);
    switch (case_identifier) {
        ...
        case 1: // max_level & 1 == 0 && ell_cuckoo == max_level + 1
            receive_standard_table(max_level, Standard_Table[(max_level - 2) >> 1]);
            break;
        case 0: // max_level & 1 == 0 && ell_cuckoo != max_level + 1
            receive_cuckoo_table(max_level, Cuckoo_Table[(max_level - (ell_cuckoo + (ell_cuckoo & 1))) >> 1]);
            break;
    }
...
void Server_0::Access() {
     ...
     */
    for (size_t i = 0; i < (written_stash ^ 1) * len_written_stash + written_stash * lg_N; i++) {
        ele = read_data_to_cell(Stash, i, element_size);
        boost::asio::write(socket_, boost::asio::buffer((ele.data()), element_size));
    }
    /**
    ...
    for (size_t i = 2; i < ell_cuckoo; i += 2) {
        if (is_index_bit_1(full_flag, i)) {
            ...
            for (size_t j = 0; j < standard_BC; j++) {
                ele = read_data_to_cell(Standard_Table[(i - 2) >> 1][level_pos_0], j, element_size);
                boost::asio::write(socket_, boost::asio::buffer((ele.data()), element_size));
                boost::asio::read(socket_, boost::asio::buffer(ele.data(), element_size));
                write_data(Standard_Table[(i - 2) >> 1][level_pos_0], j, element_size, ele);
            }
        }
    }
    /**
     ...
     */ 
    for (size_t i = ell_cuckoo + (ell_cuckoo & 1); i < max_level + 1; i += 2) {
        if (is_index_bit_1(full_flag, i)) {
            boost::asio::read(socket_, boost::asio::buffer(&level_key_0, sizeof(key_type)));
            boost::asio::read(socket_, boost::asio::buffer(&level_key_1, sizeof(key_type)));
            boost::asio::read(socket_, boost::asio::buffer(tag.data(), tag_size));
            ...
            ele = read_data_to_cell(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][0], level_pos_0, element_size);
            boost::asio::write(socket_, boost::asio::buffer((ele.data()), element_size));
            boost::asio::read(socket_, boost::asio::buffer(ele.data(), element_size));
            write_data(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][0], level_pos_0, element_size, ele);
            
            ele = read_data_to_cell(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][1], level_pos_1, element_size);
            boost::asio::write(socket_, boost::asio::buffer((ele.data()), element_size));
            boost::asio::read(socket_, boost::asio::buffer(ele.data(), element_size));
            write_data(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][1], level_pos_1, element_size, ele);
        }
    }
...
void Server_0::Rebuild(size_t level) {
    size_t level_tabLen;
    size_t case_identifier = ((level & 1) << 1) | (ell_cuckoo >= level + 1);
    switch (case_identifier) {
        case 3: // level & 1 != 0 && ell_cuckoo == level + 1
            ...
            clr(Stash);
            for (size_t i = 2; i < level; i += 2) {
                level_tabLen = parametr_c * (1 << (i - 1));
                assert(level_tabLen == Standard_Table[(i - 2) >> 1].size());
                for (size_t j = 0; j < Standard_Table[(i - 2) >> 1].size(); j++) {
                    write_element_to_socket(Standard_Table[(i - 2) >> 1][j], standard_BC);
                    clr(Standard_Table[(i - 2) >> 1][j]);
                }
            }
            /**
        ...
        case 2: // level & 1 != 0 && ell_cuckoo != level + 1
            ...
            for (size_t i = ell_cuckoo + (ell_cuckoo & 1); i < level; i += 2) {
                level_tabLen = static_cast<size_t>(parametr_c * (1 << (i - 1)) * (1 + cuckoo_epsilon));
                write_element_to_socket(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][0], level_tabLen);
                write_element_to_socket(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][1], level_tabLen);
                clr(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][0]);
                clr(Cuckoo_Table[(i - (ell_cuckoo + (ell_cuckoo & 1))) >> 1][1]);
            }
        ...
        case 1: // level & 1 == 0 && ell_cuckoo == level + 1
            ...
            receive_standard_table(level, Standard_Table[(level - 2) >> 1]);
            break;
        case 0: // level & 1 == 0 && ell_cuckoo != level + 1
            receive_shuffle_resend_cuckoo_table(level);
            receive_cuckoo_table(level, Cuckoo_Table[(level - (ell_cuckoo + (ell_cuckoo & 1))) >> 1]);
            break;
    }
...
void Server_0::RebuildMaxLevel() {
    ...
    switch (case_identifier) {
        case 3: // level & 1 != 0 && ell_cuckoo == level + 1
            ...
            write_element_to_socket(Stash, lg_N);
            clr(Stash);
            for (size_t i = 2; i < max_level + 1; i += 2) {
                level_tabLen = parametr_c * (1 << (i - 1));
                assert(level_tabLen == Standard_Table[(i - 2) >> 1].size());
                for (size_t j = 0; j < Standard_Table[(i - 2) >> 1].size(); j++) {
                    write_element_to_socket(Standard_Table[(i - 2) >> 1][j], standard_BC);
