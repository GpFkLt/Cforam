...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <log_database_sizes length> <log_database_sizes values> <element_sizes length> <element_sizes values> <0Or2>\n";
        return 1;
    }
    ...
    size_t para0Or2 = std::strtoull(argv[3 + log_database_sizes_length + element_sizes_length], nullptr, 10);
    ...
    for (size_t log_database_size : log_database_sizes) {
        for (size_t element_size : element_sizes) {
            ...
            Client client(N, element_size, tag_size, para0Or2);
            client.Setup();
            ...
            std::cout << "outFIle" << "\n";
            std::ofstream outfile("../../Result/oursopt_results.csv");
