...
const std::pair<std::string, int> Server0_IPAndPort = {"127.0.0.1", 4002};
const std::pair<std::string, int> Server1_IPAndPort = {"127.0.0.1", 4003};

Client::Client(size_t N, size_t element_size, size_t tag_size, size_t para0Or2): 
    N(N), element_size(element_size), tag_size(tag_size), para0Or2(para0Or2),
...
element_type Client::Access(char op, addr_type req_addr, uint8_t* write_value) {
    ...
    if (len_B > 1) {
        ...
        access_bandwidth += ((element_size >> para0Or2) * (len_B - 1)) + (keys_buffer.first.size() << 1); 
    }
    ...
    if (len_S > 1) {
        ...
        access_bandwidth += ((element_size >> para0Or2) * (len_S - 1)) + (keys_stash.first.size() << 1); 
    }
    ...
    if (is_index_bit_1(full_flag, 0)) {
        ...
        access_bandwidth += (keys_0.first.size() << 2) + ((element_size >> para0Or2) << 2); 
    }
    ...
    for (size_t i = ell + 1; i <= L; i++) {
        if (is_index_bit_1(full_flag, i - ell)) {
            ...
            access_bandwidth += (sizeof(size_t) << 2) + ((element_size >> para0Or2) << 2);
        }
    ...
    access_bandwidth += ((element_size >> para0Or2) << 1) + (tag_size << 1); 
...
void Client::RebuildEll() {
    ...
    for (size_t i = 1; i < len_B; i++) {
        ...
        access_bandwidth += ((element_size >> para0Or2) << 1) + (element_size >> para0Or2) + (tag_size << 1) + (sizeof(size_t) << 2); 
    }   
...
void Client::RebuildLevel(size_t level) {
    ...
    for (size_t i = 1; i < len_B; i++) {
        ...
        access_bandwidth += ((element_size >> para0Or2) << 1) + (element_size >> para0Or2) + (tag_size << 1) + (sizeof(size_t) << 3);
    }   
...
void Client::RebuildL() {
    ...
    access_bandwidth += (element_size >> para0Or2) * (all_ele_num + N);
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        access_bandwidth += ((element_size >> para0Or2) << 1) + (tag_size << 1) + (sizeof(size_t) << 3); 
    }
